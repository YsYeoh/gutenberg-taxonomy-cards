...
- **`taxonomy-category-cards`** (`gutenberg-taxonomy-cards/taxonomy-category-cards`) — the original block. Grid of **all** terms in a taxonomy the user picks (Source panel: post type → taxonomy). The richest block in the plugin — per-element text style, per-category border/animation overrides, per-term icon badges. See the dedicated sections below; everything from "Card is a link, not a div" through "Per-term icon badges" is specific to this block. It's also the only block with a block style variation so far: `block.json`'s `"styles"` array registers `default` and `overlay`, giving native "Styles" panel toggle between them with zero extra attributes or JS — WordPress just adds/removes an `is-style-overlay` class on the wrapper, which both `edit.js`'s preview and `view.js`'s hydration already carry through unmodified since neither touches the wrapper's own className. The "Overlay" style is implemented as a CSS-only reskin (`overlay-style($prefix)` in `src/shared/_grid-card.scss`, included from this block's `style.scss` only): `.__content` switches from sitting below `.__image` in normal flow to being absolutely positioned over its bottom edge with a dark gradient scrim, while `.__image` itself is untouched — so the existing image-ratio/fit controls keep working unchanged. The scrim's dark stop holds at 55–100% opacity, with a 48px `padding-top` buffer above the text before the gradient fades to transparent — the fade has to happen across that empty buffer, not across the title/description themselves, or the topmost line of text ends up in the near-transparent part of the gradient and becomes unreadable against a light image.
