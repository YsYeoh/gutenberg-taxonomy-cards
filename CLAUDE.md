...
### Per-card border and hover animation

Same reasoning as the color-support cascade above: native `border` block support would apply to the wrapper (the grid container), not the individual `.__card` boxes, so border styling uses the same custom-attribute + CSS-custom-property pattern as `cardRadius` instead — `borderWidth`/`borderStyle`/`borderColor` become `--rcc-border-width`/`--rcc-border-style`/`--rcc-border-color`, consumed by a single `border: var(...)` declaration on `.__card`. An unset `borderColor` falls back to CSS's own `currentColor` keyword (the resolved text color), not a hardcoded gray.
